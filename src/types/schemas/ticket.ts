...
export const TicketSchema = BasePostSchema.extend({
  ...
  sale_price_start_date: z.string().regex(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/, 'Date must be in Y-m-d H:i:s format (e.g., "2024-12-01 00:00:00")').optional().describe('When sale price starts (must be in Y-m-d H:i:s format)'),
  sale_price_end_date: z.string().regex(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/, 'Date must be in Y-m-d H:i:s format (e.g., "2024-12-15 23:59:59")').optional().describe('When sale price ends (must be in Y-m-d H:i:s format)'),
}).meta({
  title: 'Ticket',
